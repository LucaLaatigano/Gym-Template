    ...
    return (
        <section ref={container} id={id} className="h-auto w-full py-20 bg-studio-dark text-white">
            <div className="div-layout">
                ...
                <div ref={gridRef} className="grid-layout border border-studio-crema/20">
                    {classesData.map((item, index) => (
                        <div
                            key={index}
                            className={`
                                box-class flex items-start gap-8 p-10 bg-studio-card
                                border-b border-studio-crema/20
                                md:border-r md:border-studio-crema/20
                                ${(index + 1) % 2 === 0 ? "md:border-r-0" : ""}
                                ${index >= classesData.length - 2 ? "md:border-b-0" : ""}
                                lg:border-r lg:border-b
                                ${(index + 1) % 3 === 0 ? "lg:border-r-0" : ""}
                                ${index >= classesData.length - 3 ? "lg:border-b-0" : ""}
                            `}
                        >
